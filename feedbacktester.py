...
def delete_pools(pools, num):
	...
	for pool in sortedpools:
		#print "delete: choose pid", pool[0], "score", pool[6], "uniquness", pool[7]
...
def feedback(pool, sequences, branches, statements):
	global config, fid, R, start, num_redundancies, num_nonerrorseqs, num_errorseqs
	elapsed = time.time()
	sut = pool[1]
	seq = R.choice(pool[2])[:]
	sut.replay(seq)
	...
	#n = R.randint(2, 100) if R.randint(0, 9) == 0 or (pool[5]== 0.0 or len(sut.allBranches()) == 0 or len(sut.allStatements()) == 0) else 1
	num_skips = 0
	for i in xrange(n):
		...
		if redundant(tuple_seq, sequences):
			...
			num_skips += 1
			if n == num_skips:
				#print "feedback: seq is redundant pid", pool[0]
				num_redundancies += 1
				return True
			continue
		ok = sut.safely(a)
	...
	if not config.single:
		covered(pool)
	#print "feedback: seq is new pid", pool[0]
	num_nonerrorseqs += 1
	sequences.add(tuple_seq)
	pool[2].append(seq)
	pool[5] += (time.time() - elapsed)
...
def select_pool(pools):
	...
	for pool in pools:
		pool[6] = score(pool)
		if pool[6] == float('inf'):
			#print "select: pid", pool[0], "score", pool[6]
			return pool
		if pool[6] > maxscore:
			maxscore = pool[6]
			selected = pool
	#print "select: pid", selected[0], "score", selected[6]
	return selected
...
def main():
	global config, fid, pid, R, start, sequences, num_redundancies, num_nonerrorseqs, num_errorseqs
